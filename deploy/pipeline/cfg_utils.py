    ...
    def merge_opt(cfg, arg):
        ...
        if 'opt' in arg.keys() and arg['opt']:
            for name, value in arg['opt'].items():
                ...
                # Check if it's a top-level config (like crop_thresh, visual, etc.)
                if name in ['crop_thresh', 'attr_thresh', 'kpt_thresh', 'visual', 'warmup_frame', 'log_level']:
                    # Direct assignment for top-level configs
                    merge_cfg[name] = value
                elif isinstance(value, dict):
                    # Nested config like MOT, ATTR, etc.
                    ...
                    # Handle cases where we have single value for a module
                    # This might be an error in the input format
                    print(f"Warning: {name} has unexpected value type: {type(value)} = {value}")
